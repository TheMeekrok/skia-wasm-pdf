...
class PDFDocument {
public:
    explicit PDFDocument(sk_sp<SkDocument> doc) : doc_(doc) {}
...
class CustomSkDocument {
public:
    explicit CustomSkDocument(sk_sp<SkDocument> doc) { this->doc_ = doc; };
    sk_sp<SkDocument> doc_;
};

SkDynamicMemoryWStream* MakeSkDynamicMemoryWStream() { return new SkDynamicMemoryWStream(); }
...
EMSCRIPTEN_BINDINGS(Skia) {
    ...
    class_<SkDynamicMemoryWStream>("SkDynamicMemoryWStream")
            ...
            .function("detachAsData",
                      optional_override([](SkDynamicMemoryWStream& self) -> sk_sp<SkData> {
                          sk_sp<SkData> src_bytes = self.detachAsData();
                          return src_bytes;
                      }),
                      allow_raw_pointers());

    function("createDynamicMemoryWStream",
             optional_override(
                     []() -> SkDynamicMemoryWStream* { return new SkDynamicMemoryWStream(); }),
             allow_raw_pointers());

    function("detachStreamAsData",
             optional_override([](SkDynamicMemoryWStream* stream) -> const std::string {
                 sk_sp<SkData> data = stream->detachAsData();
                 return std::string(reinterpret_cast<const char*>(data->bytes()), data->size());
             }),
             allow_raw_pointers());

    class_<CustomSkDocument>("Document");

    function("createPDFDocument",
             optional_override([](SkDynamicMemoryWStream* stream) -> CustomSkDocument* {
                 SkPDF::Metadata metadata;
                 metadata.fTitle = "Example WritePDF() Function";
                 ...
                 auto pdfDocument = new CustomSkDocument(SkPDF::MakeDocument(stream, metadata));
                 SkASSERT(pdfDocument);
                 return pdfDocument;
             }),
             allow_raw_pointers());

    function("beginDocumentPage",
             optional_override(
                     [](CustomSkDocument* pdfDocument, int width, int height) -> SkCanvas* {
                         SkCanvas* pageCanvas = pdfDocument->doc_.get()->beginPage(width, height);
                         return pageCanvas;
                     }),
             allow_raw_pointers());

    function("closeDocumentPage",
             optional_override([](CustomSkDocument* pdfDocument) -> void {
                 pdfDocument->doc_.get()->endPage();
                 pdfDocument->doc_.get()->close();
             }),
             allow_raw_pointers());

    function("SkPDFMakeDocument",
             optional_override([](SkDynamicMemoryWStream* out_stream) -> PDFDocument* {
                 SkPDF::Metadata metadata;
